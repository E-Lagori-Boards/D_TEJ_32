...
void playhuman() {
  ...
  bool showDot = false;
  unsigned long timerPos = millis()-1000;    
  ...
  while(stayInLoop) {  // stay in loop until the player makes his/her choice hitting the OK button.
    ...
    // This is granted, because if all the cells are used, there's a winner or it's a draw.
...
void checkWinner() {    // check the board to see if there is a winner
  ...
  winner = 3;  // 3=draw, 1= winner->player, 2=winner->cpu
...
void setup() {  // function executed once, at every boot or reset.
  ...
  pinMode(BUTTON_MOVE,INPUT);    // Declaring the pin #18 as input (connected to the button move)
  pinMode(BUTTON_OK  ,INPUT);    // Declaring the pin #19 as input (connected to the button ok)   
...
void loop() {  // main loop
  ...
  if(gameStatus == 0) { // reset step.
    resetGame();   
    boardDrawing();     // drawing an empty board    
    gameStatus =   1;   // starting the game (see be HIGH)
